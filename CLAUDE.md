...
PRD §12의 나머지 오픈 퀘스천: **Q3 최종 (2026-07-08 갱신)** — 처음엔 단순 progress 레이스로 구현했으나, Codex가 **Canvas 세로 낙하 핀볼 물리 레이스**로 전면 재작업 (`src/games/race/RaceGame.tsx`, ~1,600줄). 맵 3종(클래식 콩콩맵/바람개비 지그재그/혼돈의 롱코스), 움직이는 장애물, 부서지는 벽돌 포함. 가중치는 레이스에 적용하지 않음(결과 화면 공정성 스탬프에 "물리 시뮬레이션 · 가중치 미적용" 표기). 끼임 방지 패치가 여러 번 있었으므로 **만석(25명+) 실기기 테스트 필요**.

### 8.2 추가 확정 사항 (2026-07-08)

| 항목 | 결정 |
|---|---|
| 검색 노출 | **허용** — Codex가 넣은 `noindex,nofollow` 제거함. 다른 선생님들이 검색으로 유입 가능 |
| CSP ↔ lottie-web eval 충돌 | **배포 후 실테스트로 확인** — 우리 씬은 expressions(eval 트리거)을 안 쓰므로 정상 동작 예상. Vercel 배포판에서 돌림판 당첨 연출이 안 나오면 그때 CSP `'unsafe-eval'` 추가 또는 light 빌드 교체 검토 |
| OG 이미지 | `og.png`(1.5MB) → `og.jpg`(90KB, 1200×675)로 교체. 원본은 git 히스토리에 보존 |
| 공정성 스탬프 | 결과 화면·복사 텍스트·저장 이미지에 「🔒 공정 추첨 · 방식 · 시각」 표기. 방식은 결과 종류에서 파생: 정원 추첨/레이스/사다리 = 가중치 미적용 명시, 제비/돌림판 = 가중치 유무에 따라 표기 |
| 로봇 Lottie | ~~벡터 씬 생성~~ → **폐기 (2026-07-08)**. 손수 만든 플랫 벡터 로봇이 원본 3D 렌더보다 조악하다는 사용자 피드백으로 `public/lottie/robot/` 삭제, **ChatGPT 생성 PNG 마스코트가 확정 비주얼**. 로봇 애니메이션이 다시 필요해지면 §5.1 플레이어 워크플로로 시각 검증 가능한 환경에서만 재시도할 것 |
| 레이스 재미 확장 | 사용자 선택으로 4종 물리 기믹 + 3종 연출 추가: **텔레포트 포털**(진입 시 아래쪽 랜덤 위치로 워프, 쿨다운 1.1초), **부스터/진흙 존**(레인 선택 리스크), **룰렛 게이트**(주기 개폐, 대기 중 stall 보정 제외), **트램폴린**(위로 튕겨 역주행 도박수). 연출: **실황 자막**(선두 교체·포털·대점프·골인, 1.2초 rate-limit), **포토피니시 슬로모션**(결승 320px 전 0.3×+줌 1.2×+플래시, 빨리감기 시 비활성), 선두 카메라 추적은 기존 구현 유지 |
| 레이스 공정성 검증 체계 | "같은 친구가 계속 1등" 문의 대응. 물리를 `src/games/race/engine.ts`로 분리(React/Canvas 없이 실행 가능, 컴포넌트는 이벤트 emit으로 연출만 담당) → `npm run test:race`가 **실제 게임 엔진 그대로** N판 시뮬레이션해서 우승 분포 카이제곱 검정. 공정성 근거: ①매판 출발 위치 균등 셔플 ②물리는 좌표만 참조(이름/아이콘은 렌더링 전용) ③출발 게이트 0.9초 평준화. 자연 발생 연승도 리포트에 표기 |
| 연승 배지 | 1명 당첨 게임에서 같은 친구가 연속 우승하면 결과 화면에 「🔥 N연승!」 배지 (localStorage `winStreak`로 세션 간 유지, 게임 종류 무관 이름 기준) |
| 재미 요소 6종 (사용자 선택) | ①**시상대 세레모니**: 순위 3명 이상이면 금은동 시상대(동→은→금 순차 팝) ②**랜덤 축하 멘트**: 당첨자에게 8종 멘트 중 `drawnAt` 시드로 고정 표시 ③**황금 캡슐**: 캡슐 뽑기 15% 확률 골드 연출(확률 동일, 연출만 특별) ④**사다리 동시 경주**: 전원 구슬이 색깔별로 동시에 낙하, 도착 순 공개 ⑤**BGM**: ~~8비트 루프~~ → 2.5초 반복이 거슬린다는 피드백으로 **잔잔한 앰비언트로 재작성** — 8코드×8초=64초 화성 사이클 + 코드당 1~3음 랜덤 펜타토닉 멜로디(반복감 없음), 패드 2.4초 어택/릴리즈로 이어짐, 정지 시 0.6초 페이드아웃 ⑥**미션 룰렛**: ~~미션을 돌림판에~~ → 이해 안 된다는 피드백으로 **재설계** — 돌림판엔 항상 친구 이름, 미션이 큐로 하나씩 표시("미션 1/4: 칠판 닦기 — 담당자를 돌림판으로 뽑아요!") → 스핀 → "칠판 닦기 → 김하늘 배정 완료!" → 뽑힌 친구 제외하고 다음 미션. 결과는 assignments로 저장 |
...
- [x] Vite/React 프로젝트 스캐폴딩 + 라우팅 4개 경로
- [x] `stores/session.ts` (Zustand + debounce persist) + `lib/storage.ts`
- [x] Hub 화면 (로봇 안내 영역 + 반이름/명단/가중치/제외 카드, 자동저장 상태 표시)
- [x] Lobby 화면 (상황 필터 + 게임 카드 갤러리, 제비뽑기·돌림판 우선 노출)
- [x] 제비뽑기 + 돌림판 게임 구현
- [x] 공통 결과 화면 (당첨 하이라이트 + 로봇 축하, 재추첨, 다른 게임)
- [x] Lottie: `wheel/win-burst`, `lot/reveal-paper` — 손수 제작한 벡터 씬 (파티클 버스트 14개 레이어, 쪽지 펼침 7개 레이어, 마지막 프레임 투명 처리로 잔상 없음). `robot/guide`·`robot/celebrate`·`robot/empty-state`는 **마스코트 PNG fallback 유지** (LazyLottie가 JSON 생기면 자동 교체 — 로봇 벡터화는 시각 검증이 필요해서 §5.1 플레이어 워크플로로 추후 진행)
- [x] JSON export/import, 「저장된 데이터 지우기」 확인 다이얼로그
- [ ] Vercel 배포 — `vercel.json`(SPA rewrite) 준비 완료, Vercel 대시보드에서 저장소 import 필요 (사용자 계정 작업)

### 9.2 Phase 1.5 / 2 / 3 체크리스트 (2026-07-07 완료)

- [x] **사다리타기 (1.5)**: 랜덤 가로선 자동 생성(인접 충돌 방지 + 모든 기둥쌍 최소 1개 보장), 이름 클릭→구슬 낙하 애니메이션, 전원 공개, 라벨 프리셋(당첨 1개/모둠 역할 4개), 라벨 부족 시 순환, 다시 만들기. 500회 시행 검증: 항상 순열 보장
- [x] **레이스 (2)**: 3-2-1-GO 카운트다운, 이모지 러너, 랜덤 변속, 1등/꼴찌 당첨, 코스 길이 3종, 빨리감기(×4) + 승자 확정 후 자동 가속(×3), 실시간 순위판, 전체 순위 기록
- [x] **캡슐 뽑기 (3)**: 캡슐 머신(흔들기→낙하→당첨) / 이름 릴 3개 순차 정지 모드, 한 번 더(당첨자 자동 제외), 도박 연상 완화 카피("모두에게 공평한 확률이에요")
- [x] **결과 화면 확장**: 사다리 배정 결과(라벨별 그룹), 조 배치(N명씩), 자리 배치(교탁 기준 M열) — 순위/당첨 4명 이상일 때 토글, 이미지 저장에 포함
- [x] 전 게임 reduced-motion 지원 (즉시 결과 표시)

### 9.3 Codex 이어받기 작업 (2026-07-08, 16커밋)

- [x] **정원 추첨 모드** (제비뽑기 4번째 모드): `drawUniformOrder`로 가중치 무시 균등 셔플 → 상위 N 선발 + 대기 순번. `DrawResult`에 `resultKind`/`quota` 필드 추가. 캠프 모집 초과 케이스 대응
- [x] **레이스 핀볼 재작업** (§8 Q3 갱신 참고)
- [x] `/terms`·`/privacy` 법적 페이지 + Footer
- [x] OG 메타태그 + 이미지 (이후 og.jpg 90KB로 압축)
- [x] vercel.json 보안 헤더 (CSP, X-Frame-Options, Referrer-Policy 등)
- [x] Vite 5→8 (rolldown), @vitejs/plugin-react 6
- [x] 사다리 커튼 공개 Lottie 씬(`ladder/curtain-reveal`) + 결과 숨김 개선
- [x] 모바일 반응형, LazyLottie CJS interop 수정, Hub 백업 불러오기 문구 명확화

### 9.1 구현 노트 (2026-07-07)

- Zustand `persist` 미들웨어 대신 **store 구독 + 400ms debounce + `storage.ts` 단일 진입점** 구조로 구현 (P-01·P-12의 의도인 "자동 저장·단일 진입점"을 만족하면서 savedAt 무한루프를 피하는 더 단순한 구조).
- `GameSettings.lot`에 `mode: 'one'|'many'|'order'` 필드 추가 (PRD T-01의 1명/N명/전원을 UI로 구분하기 위함).
- 추첨 로직 검증 완료: 가중치 파싱, 동명이인 id(#2), 중복 없는 뽑기, 가중 확률(2만회 시뮬레이션 오차 <1%).
- 돌림판은 rAF 기반 cubic ease-out 스핀 + 칸 경계 통과 시 WebAudio tick, `prefers-reduced-motion`이면 즉시 착지.
