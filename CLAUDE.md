...
PRD §12의 나머지 오픈 퀘스천: **Q3 결정 완료 (2026-07-07)** — 레이스는 신답룰렛식 Canvas 물리·프리셋 맵 대신 **랜덤 변속 progress 레이스**(rAF, 러버밴드 없는 개별 속도 노이즈)로 구현. "맵"은 코스 길이(짧게/보통/길게)로 단순화. 캐릭터는 이모지 러너. 가중치는 레이스에 적용하지 않음(시각적 공정성 훼손 방지, UI에 고지).
