...
| 로봇 Lottie | `robot/guide`(loop, 손 흔들기+깜빡임+안테나 펄스), `robot/celebrate`(once, 양팔 들기+별, 마지막 프레임 유지), `robot/empty-state`(guide 재사용) 벡터 씬 생성. **마음에 안 들면 `public/lottie/robot/` 폴더 삭제 시 마스코트 PNG로 자동 복귀** |
...
| 재미 요소 6종 (사용자 선택) | ①**시상대 세레모니**: 순위 3명 이상이면 금은동 시상대(동→은→금 순차 팝) ②**랜덤 축하 멘트**: 당첨자에게 8종 멘트 중 `drawnAt` 시드로 고정 표시 ③**황금 캡슐**: 캡슐 뽑기 15% 확률 골드 연출(확률 동일, 연출만 특별) ④**사다리 동시 경주**: 전원 구슬이 색깔별로 동시에 낙하, 도착 순 공개 ⑤**BGM**: WebAudio 신스 8비트 루프(저작권 无), 헤더 🎵 토글 ⑥**미션 룰렛**: 돌림판에 미션 목록 모드 + 미션→담당자 뽑기 연계, 결과는 배정(assignments)으로 저장 |
